...
\label{sec:relation_to_gr}

The Newtonian polarization calculation has a natural interpretation as the
weak-field, exterior limit of general relativity.  The approximation is local
to the vacuum or nearly vacuum buffer regions surrounding the mouths and the
planet.  The throat-supporting region may contain large curvature and exotic
stress--energy; its detailed structure is represented, from the exterior point
of view, by matching conditions, junction conditions, and response
coefficients.  The purpose of this section is to derive the scalar equation
used above from the linearized Einstein equations and to identify which parts
of the monopole response are fixed by weak-field gravity alone.

\subsection{Linearized field equations}

In an exterior buffer region choose asymptotically inertial coordinates and
write
\begin{equation}
    g_{\mu\nu}=\eta_{\mu\nu}+h_{\mu\nu},
    \qquad |h_{\mu\nu}|\ll1.
...
\end{equation}
be the trace-reversed perturbation.  In harmonic gauge,
\begin{equation}
    \partial^\mu\bar h_{\mu\nu}=0,
\end{equation}
the Einstein equations reduce, to first order in the perturbation, to
\begin{equation}
    \Box\bar h_{\mu\nu}
    =
...
\end{equation}
For a stationary source whose internal velocities are small compared with
\(c\),
\begin{equation}
    T_{00}\simeq \rho c^2,
    \qquad
    ...
    |T_{ij}|\ll T_{00}.
\end{equation}
The dominant component of \eqref{eq:linearized_einstein} is therefore
\begin{equation}
    \nabla^2\bar h_{00}
    =
    -\frac{16\pi G}{c^2}\rho .
    \label{eq:hbar00_poisson}
\end{equation}
Define the Newtonian potential by
\begin{equation}
    \bar h_{00}=-\frac{4\Phi}{c^2}.
    \label{eq:Phi_from_hbar}
\end{equation}
Then \eqref{eq:hbar00_poisson} becomes
\begin{equation}
    \boxed{\nabla^2\Phi=4\pi G\rho.}
    \label{eq:poisson_from_linearized_gr}
\end{equation}
The Poisson equation used in the scalar model is thus the Newtonian limit of
the linearized Einstein equations in the weak exterior regions.

When exterior anisotropic stress is negligible, the temporal and spatial
scalar perturbations are determined by the same potential.  The metric may be
written as
\begin{equation}
\begin{split}
    \mathrm{d}s^2={}&
...
\end{equation}
For a slowly moving test body, the spatial part of the geodesic equation gives
\begin{equation}
    \frac{\mathrm{d}^2x^i}{\mathrm{d}t^2}
    =
    -c^2\Gamma^i{}_{00}
    =
    \frac{c^2}{2}\partial_i h_{00}
    =
    -\partial_i\Phi,
    \label{eq:newtonian_acceleration_from_h}
\end{equation}
fixing the sign convention used throughout the Newtonian sections.

\subsection{Vacuum buffer regions and mouth monopoles}

Outside the planet, the mouths, and the material supporting the throat, the
leading weak-field equation is harmonic:
\begin{equation}
    \rho=0,
    \qquad
    \nabla^2\Phi=0.
\end{equation}
In a buffer region around a compact nearly spherical mouth \(i\), the general
stationary solution can be expanded as
\begin{equation}
    \Phi_i
    =
    U_i
    ...
    +\Phi_i^{\rm tidal}.
    \label{eq:buffer_multipole_expansion}
\end{equation}
Here \(U_i\) is the externally imposed potential at the mouth center, with the
mouth's own induced field excluded.  The term \(\Phi_i^{\rm tidal}\) denotes
regular external multipoles, while \(\delta M_i\) is the coefficient of the
local \(1/r_i\) field.  In the weak relativistic expansion this coefficient is
the perturbation of the local Schwarzschild mass parameter, or equivalently
the ADM or Komar mass perturbation associated with that exterior end when the
corresponding asymptotic construction is available.  In terms of the metric
perturbation,
\begin{equation}
    h_{00}^{(i)}
    =
    -\frac{2}{c^2}
    \left(
    U_i-\frac{G\delta M_i}{r_i}+\cdots
    \right).
\end{equation}
Thus the apparent mouth monopoles used in the Newtonian model are the
weak-field mass monopoles of the exterior metric.

\subsection{Lapse matching at synchronized mouths}

For a static mouth, the lapse is
\begin{equation}
    N_i=\sqrt{-g_{tt}^{(i)}}
    =
    1+\frac{\Phi_i}{c^2}+O(c^{-4}).
\end{equation}
If the two mouth worldtubes are identified using the same internal time label,
and if no additional redshift layer is inserted into the gluing map, continuity
of the induced metric requires equality of the pulled-back lapse:
\begin{equation}
    N_A(\Omega)=N_B(\mathcal O\Omega).
\end{equation}
At leading weak-field order this becomes
\begin{equation}
    \Phi_A(\Omega)
    =
    \Phi_B(\mathcal O\Omega)
    +O(c^{-2}\Phi^2).
    \label{eq:lapse_matching_phi}
\end{equation}
Equation~\eqref{eq:lapse_matching_phi} is the weak-GR origin of the
equal-potential boundary condition used in the scalar polarization model.  It
is a condition on the induced metric, specifically on \(g_{tt}\) or
\(h_{00}\).  It does not determine the normal derivatives of the metric.  Those
normal derivatives are part of the gravitational flux data and are controlled
by the stress tensor and equation of state of the throat-supporting material.

\subsection{Induced monopoles in the linearized metric}

Retain only the monopole terms and let the two nearly spherical mouths have a
common radius \(a\) and separation \(L\gg a\).  Let \(U_A\) and \(U_B\) denote
the external potentials at the mouth centers, excluding the induced mouth
fields.  On the surface of mouth \(A\), the potential entering \(h_{00}\) is,
to monopole order,
\begin{equation}
    \Phi_A^{\rm surf}
    =
...
\begin{equation}
    ...
    -\frac{G\delta M_A}{L}.
\end{equation}
Because \(h_{00}=-2\Phi/c^2\), equality of the pulled-back lapse is equivalent,
at this order, to equality of these surface potentials:
\begin{equation}
    U_A
    -\frac{G\delta M_A}{a}
    -\frac{G\delta M_B}{L}
    ...
    -\frac{G\delta M_B}{a}
    -\frac{G\delta M_A}{L}.
    \label{eq:GR_monopole_matching}
\end{equation}
This relation fixes only the difference of the induced monopoles.  A second
condition specifies the global homogeneous mode and the bookkeeping convention
for total mass.  In the idealized fixed-total-mass, zero-added-flux convention,
\begin{equation}
    \delta M_A+\delta M_B=0.
    \label{eq:GR_zero_sum}
\end{equation}
Equations~\eqref{eq:GR_monopole_matching} and \eqref{eq:GR_zero_sum} give
\begin{equation}
    \boxed{
    \delta M_A
    =
    ...
    \label{eq:GR_induced_monopoles}
\end{equation}
For \(L\gg a\),
\begin{equation}
    \delta M_A\simeq \frac{a}{2G}(U_A-U_B),
    \qquad
    \delta M_B\simeq-\delta M_A.
\end{equation}
If a planet of mass \(M\) is close to mouth \(A\), with \(D_A=D\), while mouth
\(B\) is far away, then
\begin{equation}
    U_A\simeq-\frac{GM}{D},
    \qquad
...
\end{equation}
The near mouth then carries a negative induced mass monopole and the distant
mouth an equal positive one in this ideal response convention.  The result is a
redistribution of the exterior weak-field monopole, not a creation of net
asymptotic mass.

\subsection{Thin-shell junctions and response coefficients}

For a cut-and-paste wormhole, the induced metric \(q_{ab}\) on the throat is
continuous, while a jump in extrinsic curvature is supported by a surface
stress tensor:
\begin{equation}
    \boxed{
    S_{ab}
    ...
    \label{eq:israel}
\end{equation}
These Israel junction conditions relate the discontinuity in the normal
derivatives of the metric to the surface energy density, pressure, and tension
of the throat.  In the weak-field description, continuity of \(q_{tt}\) gives
the lapse matching condition, while the jump in \(\partial_n h_{\mu\nu}\)
contains the gravitational flux response of the supporting shell.

The preceding derivation therefore fixes the exterior Poisson equation, the
identification of \(1/r\) coefficients with weak-field mass monopoles, and the
equal-lapse condition for synchronized mouths.  It does not fix a universal
susceptibility.  In a more general throat model one should write
\begin{equation}
    \delta M_A=-\delta M_B
    =
    \chi_0(U_A-U_B),
...
\end{equation}
where \(\chi_0\) is determined by the throat geometry, supporting fields,
external constraints, and choice of homogeneous mode.  The ideal scalar model
corresponds to
\begin{equation}
    \chi_0^{\mathrm{ideal}}
    =
...
\subsection{Status of the relativistic extension}

The following ingredients have a controlled weak-field or junction-condition
basis:
\begin{itemize}
    \item the reduction of the linearized Einstein equations to
    \(\nabla^2\Phi=4\pi G\rho\) in weak exterior regions;
    \item the Newtonian acceleration law
    \(\mathrm{d}^2\mathbf x/\mathrm{d}t^2=-\boldsymbol\nabla\Phi\) for slow test
    bodies;
    \item the interpretation of a local \(1/r\) term in \(\Phi\) as a
    weak-field mass monopole;
    \item equality of lapse, and hence equality of \(\Phi\) to leading order,
    at synchronized identified mouth points;
    \item support of jumps in the normal derivatives of the metric by shell
    stresses through the Israel junction conditions.
\end{itemize}

The following ingredients are constitutive assumptions of the effective model:
\begin{itemize}
    \item the value of the susceptibility \(\chi_0\);
    \item the fixed-total-monopole condition
    \(\delta M_A+\delta M_B=0\), rather than a change in homogeneous throat flux
    or stored throat energy;
    \item rigidity of the mouths under acceleration or tidal loading;
    \item the division of recoil, stored energy, and stress between the two
    mouths;
    \item stability of any passive gravitational-conductor-like response.
\end{itemize}
